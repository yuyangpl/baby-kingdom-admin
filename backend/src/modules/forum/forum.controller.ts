...
export async function updateCategory(req: Request, res: Response): Promise<void> {
  const cat = await forumService.updateCategory(req.params.id, req.body, (req as any).user.id, req.ip);
  success(res, cat);
}
...
export async function updateBoard(req: Request, res: Response): Promise<void> {
  const board = await forumService.updateBoard(req.params.id, req.body, (req as any).user.id, req.ip);
  success(res, board);
}

export async function updateBoardPersonas(req: Request, res: Response): Promise<void> {
  const board = await forumService.updateBoardPersonas(req.params.id, req.body.personaBindings, (req as any).user.id, req.ip);
  success(res, board);
}

export async function deleteBoard(req: Request, res: Response): Promise<void> {
  await forumService.deleteBoard(req.params.id, (req as any).user.id, req.ip);
  success(res, null);
}
