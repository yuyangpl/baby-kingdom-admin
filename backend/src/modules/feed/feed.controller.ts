...
export async function updateContent(req: Request, res: Response): Promise<void> {
  const { content, subject, toneMode, personaId, adminNotes } = req.body;
  if (!content) throw new ValidationError('Content is required');
  const feed = await feedService.updateContent(req.params.id as string, { content, subject, toneMode, personaId, adminNotes }, (req as any).user.id, req.ip ?? '');
  success(res, feed);
}
