...
| `GEMINI_API_KEY` | Google Gemini AI API 密钥 | Backend (Cloud Run env) |
...
gcloud run services update babykingdom-backend --region=us-central1 \
  --update-secrets=DATABASE_URL=DATABASE_URL:latest,JWT_SECRET=JWT_SECRET:latest,ENCRYPTION_KEY=ENCRYPTION_KEY:latest,GEMINI_API_KEY=GEMINI_API_KEY:latest
```
